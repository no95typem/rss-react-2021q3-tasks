...
describe('wh-api', () => {
  it('should mod query and transform query to string', () => {
    ...
    expect(q).toBe(
      `search?q=test&categories=011&purity=111&sorting=favorites&page=5&apikey=x`,
    );
  ...
  it('should throw an error when search query is not valid', async () => {
    ...
    const error = await getWHSearch(query).catch(err => err);
  ...
  it('should fetch when search query is valid', async () => {
    ...
    const result = await getWHSearch(DEFALUT_WHQUERY).catch(err => false);
